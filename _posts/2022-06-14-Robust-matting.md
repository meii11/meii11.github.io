...
作者发现在一半通道上（一般使用降维操作，作者写的是分离和聚合操作）上使用ConvGRU是高效且有效的（effective and effience）。这种设计有助于 ConvGRU 专注于聚合时间信息，而另一个拆分分支则转发特定于当前帧的空间特征。除了最后一个卷积使用了1 $\times$ 1的卷积核，其余的卷积都是3 $\times$ 3。
...
RobustMatting 模型基于如下几个原因在抠图和语义物体分割任务上进行训练：

1. 人像抠图和人像分割任务是十分关联的，和三元方法和基于背景的方法需要额外的辅助输入不同的是，作者模型需要能够从语义的角度真正的理解场景，并且在人像定位上更加鲁棒；
2. 目前大多数数据集仅提供必须合成（synthetically composited）到背景图像的真值alpha和前景图，但是由于前景和背景的亮度不同所以看起来会很假。另一方面，语义分割数据集以真实图像为特征，其中人类主体包含在所有类型的复杂场景中。使用这样的数据集进行训练可防止模型过度拟合合成分布。
3. 语义分割任务有很多数据集可用（感觉这才是关键 - - ！）

### Dataset

作者使用的抠图数据集为VideoMatte240K、Distinctions-606以及Adobe Image Matting；使用的语义分割数据集为YoutubeVIS、COCO和SPD。

### Procedure

训练过程看起来其实有点冗杂了，不过这可能就是作者所说的创新训练方式吧。大致就是将训练分为4个步骤，每个步骤使用了不同的学习率以及帧率（这些步骤是在抠图数据集上进行训练的），而作者所说的将抠图和语义分割任务进行结合训练的方法是，在上述四个步骤走过一轮后插入语义分割训练，例如在奇数轮次使用图像分割数据，在偶数轮次使用视频分割数据。这里就不详细展开了。

### Losses

这部分需要专门整理一下，这里先简单整理一下。因为有两组真值，分别为 $\alpha_{t}$ 和其对应的真值 $\alpha_{t}^{*}$ ，以及前景 $F_t$ 和对应真值$F_t^*$ 。所以针对两者均有不同的损失函数，有对alpha值的L1 Loss、pyramid Laplacian loss、temporal coherence loss，公式为：

![截屏2022-06-15 下午4.14.29](/assets/images/md_image/RobustMatting/fomula1.png)
